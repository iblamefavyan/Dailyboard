...
export async function ambilCuaca(kota) {
    ...
    if (!lokasiData.results?.length) {
        throw new Error("Kota tidak ditemukan");
    }

    const lokasi = lokasiData.results[0];

    const cuacaRes = await fetch(
        `https://api.open-meteo.com/v1/forecast?latitude=${lokasi.latitude}&longitude=${lokasi.longitude}&current=temperature_2m&timezone=auto`
    );

    const cuacaData = await cuacaRes.json();

    return `${lokasi.name}: ${cuacaData.current.temperature_2m}°C`;
}